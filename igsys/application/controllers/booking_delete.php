...
/*	UPDATE A BOOKING*/	
	function update_booking()
	{
		...
		$result = $this->booking_model->update_booking($bookingNumber,$customerReferral,$propertyList,$customerSpecials,$fromTime,$adults,$children,$infants,$toTime,$cot,$highchair,$bookingNotes,$bookingFee,$customerPrice,$customerBalance,$commissionableCost,$commissionPercentage,$commissionAmount,$agentFee,$ownerBalance,$ownerPaid,$ownerPaidDate,$ownerReference,$ownerPaymentMethod,$repeatBusiness,$bookingStatus);
		...
		{
			$this->view_booking($bookingNumber);
		}
		else
		{
			$this->edit_booking($bookingNumber);
		}
	}
...
/*	ADD A CHARGE*/
	function add_charge()
	{
		...
		$data['paymentId'] = $this->charges_model->add_charge($bookingNumber,$chargePurpose,$chargeAmount);
		$this->booking_model->add_charge($bookingNumber,$customerPrice,$customerTotalCharges,$customerBalance,$commissionableCost,$commissionAmount,$agentFee,$ownerBalance);
		$this->edit_booking($bookingNumber);
	}
...
/*	LIST BOOKINGS */
	function list_bookings($status)
	{
		...
		$data['yearCombo'] = $this->global_model->get_year_combo();
		$headerView = $this->global_model->get_standard_header_view();
