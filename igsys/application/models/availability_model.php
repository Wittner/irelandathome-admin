...
class Availability_model extends Model
{
  ...
  function getLatestAvailability($townId, $filter, $calendarDate,$rooms,$sleeps,$code)
  {
    $this->load->database();
    $rootPage = base_url();
    ...
    // First get town name for table header (Only if we are not using the IAH Managed filter)
    if($filter != 'iah') {
    ...
    if($filter === 'iah') {
      $this->public_db->where('iah_availability =', 'on');
    }
    if($code != 'any') {
        ...
        for ($co = 1; $co <= $days_in_month; $co++)
        {
          ...
          if ($query->num_rows() > 0)
          {
            foreach ($query->result_array() as $row)
            {
              ...
              $availAlloc = $row['availAlloc'];
              $remainderAlloc = $propAlloc[$key] - $availAlloc;
              if ($remainderAlloc <= 0)
              {
                // No availability
  ...
  function add_availability($propertyCode,$fromDate,$numberNights)
  {
    // First check if required availability is actually available TBD
  ...
  function add_availability_all($propertyCode,$fromDate,$customerNights)
  {
    // First check if required availability is actually available TBD

    // Work out all the dates from arrival to departure
    // Get property's allocation
    $propertyUnits = $this->property_model->get_property_detail($propertyCode,'property_units');
    $list='';
    $splitFromDate=explode("-",$fromDate);
    $fromDay = $splitFromDate[2];
    $fromMonth = $splitFromDate[1];
    ...
    for($co=0;$co<=($customerNights-1);$co++)
    {
      ...
      if ($query->num_rows() >= '1')
      {
        $query = $this->public_db->query("update availability set availAlloc='$propertyUnits' where availDate='$insertDate' and availPropertyCode='$propertyCode'");
      }
      else
      {
        $query = $this->public_db->query("insert into availability (availId, availPropertyCode, availStatus, availAlloc, availDate)values('', '$propertyCode', 'BOOKED', '$propertyUnits', '$insertDate') ");
      }
  ...
  function release_availability($propertyCode,$fromDate,$numberNights)
  {
    ...
    for($co=0;$co<=($numberNights-1);$co++)
    {
      ...
      if ($query->num_rows() >= '1')
      {
        foreach ($query->result_array() as $row)
        {
          $availChk = $row['availAlloc'];
          if($availChk == 1)
          {
            $query = $this->public_db->query("delete from availability where availDate='$insertDate' and availPropertyCode='$propertyCode'");
          }
          else
          {
            $query = $this->public_db->query("update availability set availAlloc=availAlloc-1 where availDate='$insertDate' and availPropertyCode='$propertyCode'");
  ...
  function release_availability_single($propertyCode,$fromDate,$numberNights)
  {
    // First work out all the dates from arrival to departure
    $list='';
    $splitFromDate=explode("-",$fromDate);
  ...
  function release_availability_all($propertyCode,$fromDate,$numberNights)
  {
    // First work out all the dates from arrival to departure
    $list='';
    $splitFromDate=explode("-",$fromDate);
